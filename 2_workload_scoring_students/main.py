...
for user in users:
    workloadScores = lib_main.workloadScoringByStatuses(DataFrame[DataFrame.assignee_id == user][:], 63, 7)
    status_scores_table = pd.concat([status_scores_table, workloadScores])
    total_scores_table = pd.concat([total_scores_table, lib_main.get_total_score(workloadScores)])
...
lib_main.insertScoreResultData(status_scores_table, 'findcsystem', 'xsolla_summer_school', 'score_result_status')
